...
from pleiotropy.evolution.ocra.neat.activations import ActivationFunctionSet
from pleiotropy.evolution.ocra.neat.aggregations import AggregationFunctionSet
from pleiotropy.evolution.ocra.neat.config import ConfigParameter, write_pretty_params
from pleiotropy.evolution.ocra.neat.genes import DefaultNodeGene, DefaultConnectionGene
from pleiotropy.evolution.ocra.neat.graphs import creates_cycle
...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, params):
        ...
        self._params = [ConfigParameter('num_inputs', int),
                        ...
                        ConfigParameter('genome_seed', str, 'none')]
    ...
    def get_new_node_key(self, node_dict):
        if self.node_indexer is None:
            self.node_indexer = count(max(node_dict.keys()) + 1)
...
class DefaultGenome(object):
    ...
    def configure_new_base(self, config):
    ...
    def configure_new_recurrent_morph_control_path(self, config):
        # Add two hidden nodes
        hidden1_key = config.get_new_node_key(self.nodes)
        self.nodes[hidden1_key] = self.create_node(config, hidden1_key)
        hidden2_key = config.get_new_node_key(self.nodes)
        self.nodes[hidden2_key] = self.create_node(config, hidden2_key)
        hidden3_key = config.get_new_node_key(self.nodes)
        self.nodes[hidden3_key] = self.create_node(config, hidden3_key)

        # Add connections
        x, y, r = config.input_keys
        p, m, o, f = config.output_keys
        ...
        connections = [
            ...
            # (r, hidden1_key),
            # Hidden1 -> Hidden2
            (hidden1_key, hidden2_key),
            # Hidden2 -> Hidden1
            (hidden2_key, hidden1_key),
            # Hidden2 -> (Morph, Controller)
            (hidden2_key, p),
            (hidden2_key, m),
            (hidden2_key, o),
            (hidden2_key, f),
            # Hidden3 -> (Morph, Controller)
            (hidden3_key, p),
            (hidden3_key, m),
            (hidden3_key, o),
            (hidden3_key, f),
        ...
        for src, targ in connections:
            connection = self.create_connection(config, src, targ)
            self.connections[connection.key] = connection

    def configure_new_shared_morph_control_path(self, config):
        # Add two hidden nodes
        hidden1_key = config.get_new_node_key(self.nodes)
        self.nodes[hidden1_key] = self.create_node(config, hidden1_key)
        hidden2_key = config.get_new_node_key(self.nodes)
        ...
        connections = [
            ...
            (y, hidden2_key),
            (r, hidden2_key),
            # Hidden1 -> (Morph1, all controller)
            (hidden1_key, p),
            (hidden1_key, o),
            (hidden1_key, f),
            # Hidden2 -> (Morph2, all controller)
            (hidden2_key, m),
            (hidden2_key, o),
            (hidden2_key, f)
        ]
    ...
    def configure_new(self, config):
        ...
        if config.genome_seed == 'none':
            self.configure_new_base(config)
        elif config.genome_seed == 'shared_morph_control_path':
            self.configure_new_shared_morph_control_path(config)
        elif config.genome_seed == 'recurrent_morph_control_path':
            self.configure_new_recurrent_morph_control_path(config)
    ...
    def mutate_add_connection(self, config):
        ...
        # > in_node in config.output_keys and out_node in config.output_keys:
...
GenomeType = Union[DefaultGenome, Tuple[DefaultGenome, DefaultGenome], Tuple[
    DefaultGenome, DefaultGenome, DefaultGenome, DefaultGenome]]
