...
#     return mat
from ..AESHN.es_hyperneat.utils import GNeuron, GConnection
...
class RecurrentNet():
    def __init__(self, n_inputs, n_hidden, n_outputs,
                 ...
                 n_internal_steps=1,
                 dtype=torch.float64, num_cons=0):
    ...
    def activate(self, inputs: np.ndarray) -> np.ndarray:
        ...
        return self.outputs.numpy().flatten()

    @staticmethod
    def create(input_keys: List[Tuple[float, float]], output_keys: List[Tuple[float, float]],
               nodes: Dict[Tuple[float, float], GNeuron],
               connections: Dict[Tuple[Tuple[float, float], Tuple[float, float]], GConnection], batch_size=1,
               activation=sigmoid_activation,
        ...
        required = required_for_output(
            input_keys, output_keys, connections)
        if prune_empty:
            nonempty = {conn.key[1] for conn in connections.values() if conn.enabled}.union(
                set(input_keys))

        input_keys = list(input_keys)
        hidden_keys = [k for k in nodes.keys()
                       if k not in output_keys and k not in input_keys]
        output_keys = list(output_keys)

        hidden_responses = [nodes[k].response for k in hidden_keys]
        output_responses = [nodes[k].response for k in output_keys]

        hidden_biases = [nodes[k].bias for k in hidden_keys]
        output_biases = [nodes[k].bias for k in output_keys]
        ...
        for conn in connections.values():
            ...
            if o_key not in required and i_key not in required:
                removed_connection_keys.add(conn.key)
                continue
        ...
        return RecurrentNet(n_inputs, n_hidden, n_outputs,
                            ...
                            n_internal_steps=n_internal_steps, num_cons=num_cons), removed_connection_keys
